...
头文件. *.c . main.c 之间的桥梁. 引用了对应的头文件, 才能调. *.c 中具体的实现函数.

*.h 类似于Java中接口(interface) 
...
结构体和Java中的类相似(多数操作都能对应上):

tag为类名

variable-list为对象名

可以在定义variable-list的时候赋值
...
2. 结构体变量定义, 变量可为指针, 数组, 普通变量, 上面的variable-list是在结构体定义时声明的
...
所以swap2的结果可以解释。值得注意的是，由于引用时别名，所以引用并不是一种数据类型，内存并不会给它单独分配内存，而是直接调用它所引用的变量。
